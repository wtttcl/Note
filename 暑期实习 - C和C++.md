...
3. 当 reserve() 新设置的 capacity 比原空间⼩时，是不会引起重新分配的。
...
- 可以用于任何表达式，包括没有初始化的变量，但不计算表达式。

- 如果 decltype使用的是一个不加括号的变量，则得到的结果就是该变量的类型;如果给变量加上了一层或多层括号，则编译器将推断得到引用类型。

  ```
  例如int &r = i;则decltype(r)的类型为int &，要想得到int类型需要decltype(i + 0)。
  
  4、如果表达式的内容是解引用操作，则将得到引用类型，例如int *p = &i;则decltype(*p)的类型时int &。
  
  5、decltype((variable))得到的类型永远是引用，
  ```
